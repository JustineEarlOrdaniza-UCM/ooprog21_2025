...
public class DebugFive3
{
   public static void main (String args[])
   {
      ...
      if(item < LOW) // change "if(item > LOW)" to "if(item < LOW)"
      output = "Item number too low";
      
      else if(item > HIGH) // change "if(item >= HIGHH)" to "if(item > HIGH)"
      output = "Item number too high";
      
      else if(item < CUTOFF) // change "if(item == CUTOFF)" to "if(item < CUTOFF)"
